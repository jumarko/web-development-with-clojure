...
The `home-routes` handler in the `picture-gallery.routes.home` namespace
...
The `home-page` function will in turn call the `picture-gallery.layout/render` function
...
The rest of this page is rendered by ClojureScript found in the `src/cljs/picture_gallery/core.cljs` file.



#### Organizing the routes

The routes are aggregated and wrapped with middleware in the `picture-gallery.handler` namespace:
...
* Let `mount` know to start the database connection by `require`-ing `picture-gallery.db.core` in some other namespace.
